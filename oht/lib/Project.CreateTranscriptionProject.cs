...
namespace oht.lib
{
    public interface ICreateTranscriptionProjectProvider
    {
        string Get(string url, WebProxy proxy, string publicKey, string secretKey, string sourceLanguage
			, string sources, string wordcount = "", string notes = "", string callbackUrl = "", string name = "", string referenceResources = "", string[] custom = null);
    }
    public class CreateTranscriptionProjectProvider : ICreateTranscriptionProjectProvider
    {
			...
			, string sources, string wordcount = "", string notes = "", string callbackUrl = "", string name = "", string referenceResources = "", string[] custom = null)
        {
            using (var client = new WebClient())
            {
                ...
                var web = url + String.Format("/projects/transcription?public_key={0}&secret_key={1}&source_language={2}&sources={3}&wordcount={4}"
                    , publicKey, secretKey, sourceLanguage, sources, wordcount);
    ...
    partial class Ohtapi
    {
        ...
        /// <param name="wordcount">[Optional] Integer of seconds, if empty use automatic counting</param>
			...
			, string sources, string wordcount = "", string notes = "", string callbackUrl = "", string name = "", string referenceResources = "", string[] custom = null)
        {
            ...
            try
            {
				...
				var json = CreateTranscriptionProjectProvider.Get(Url, _proxy, KeyPublic, KeySecret, sourceLanguage, sources, wordcount, notes, callbackUrl, name, referenceResources, custom);
